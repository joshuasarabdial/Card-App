...
/** Function that takes a list and iterates through and deletes its nodes plus its data.
 * Relies on the list's delete function to handle deleting its nodes' data.
 *@post list head and tail are set to NULL and length is set to 0
...
void clearList(List* list) {
  ...
  ListIterator itr = createIterator(list);
  Node* theNode = itr.current;
  void* theData = NULL;

  while ((theData = nextElement(&itr)) != NULL) {
    list->deleteData(theData);
    free(theNode);
    theNode = itr.current;
  }
...
/** Function that creates an iterator for a list data struct. The iterator's current nodes
 *  will point to the list's head if it exists.
 *@post list remains unchanged
 *@return ListIterator data structure
 *@param list - a pointer to the list structure
 **/
...
/** Function that takes a list iterator and sets it to the next node in the list.
  * It will return the data from the current node.
  *@post iterator current will be the list node's next
  *@return the data from the iterator's current nodes
  *@param list iterator - a pointer to the iterator structure
  **/
